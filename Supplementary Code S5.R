                         ...
                         VarOrder = NULL, Intercept = TRUE, Size = 2, tips = 0.2) 
{
  ...
  for (i in 1:length(ModelList)) {
    ...
    if (class(model) == "inla") {
      ...
      Graph <- Graph[1:6,] ## edit Valentina
...
#Import the data 
mydata <- read.csv(file = "PresenceAbsence_casesINLA_75km.csv", 
                   header = TRUE,
                   na.strings = "NA",
...
dim(mydata)
# remove NAs
mydata <- mydata[!is.na(mydata$soil_pH),]
dim(mydata)
...
##################################################################
# Underlying task. 
# Model total anthrax outbreaks as a function of: 
#   -bio2, bio6, bio17, bio18. 
#   -Elevation
#   -Distance to water
#   -Soil calcium and soil pH 
#   -Soil water
...
## mydata
###################
# Basis functions for the bio18  data:
...
###################
# Basis functions for the bio2  data:
...
Efxplot(list(Mb8.c), 
        ModelNames = c(""))
## Plot new fixed effects
Efxplot.Val(list(Mb8.c), 
            ModelNames = c(""))

# Covariate effects
round(Mb8.c$summary.fixed[, c("mean", "0.025quant", "0.975quant")], 3)
#Mb17.c$summary.fitted.values[320:420,]
###################


## Smoothers**** 
## Page 524 V.2 for easier to interpret smoothers
Ns <- nrow(mydata)
...
Is.elevation <- order(mydata$elevationc)
...
p <- p + geom_line(data = MyData, 
                   aes(x = Xaxis, y = mu), color="dodgerblue3", size = 0.6)
...
### Probability of occurrence curves for linear predictors (v.1: p.182)
Xm.p <- data.frame(
  Intercept              = rep(1, N),
  outbreaks              = mydata[,"outbreaks01"],
  bio4                   = mydata[,"bio4"],
...
## Bio12
MyData <- data.frame(
  distance_to_water = seq(from = min(Xm.p$distance_to_water),
                           to   = 0.3, # max(Xm.p$bio12),
                           length = 100))

MyData
Xmat <- model.matrix(~ distance_to_water,
                     data = MyData)
head(Xmat)
Xmat <- as.data.frame(Xmat)
lcb <- inla.make.lincombs(Xmat)

Mb5.p <- inla(outbreaks ~ distance_to_water,
           lincomb = lcb,
...
Pred.pm <- Mb5.p$summary.lincomb.derived[,c("mean", 
                                         "0.025quant", 
                                         "0.975quant")]
...
MyData$mu <- unlist( 
  lapply(
    Pred.marg,
    function(x) inla.emarginal(MyFun, x)))
...
# Plot v.1:182   
p <- ggplot()
p <- p + geom_point(data = Xm.p, 
                    aes(y = outbreaks, x = distance_to_water),
                    shape = 1, 
                    size = 1)
p <- p + xlab("Distance to Water") + ylab("Probability of occurrence")
p <- p + theme(text = element_text(size=18)) 
p <- p + geom_line(data = MyData, 
                   aes(x = distance_to_water, 
                       y = mu), 
                   colour = "navyblue",
                   size = 1.0)
...
## Distance to water
MyData <- data.frame(
  bio4 = seq(from = min(Xm.p$bio4),
              to   = max(Xm.p$bio4),
              length = 100))

MyData
Xmat <- model.matrix(~ bio4,
                     data = MyData)
head(Xmat)
Xmat <- as.data.frame(Xmat)
lcb <- inla.make.lincombs(Xmat)

Mb5.p <- inla(outbreaks ~ bio4,
              lincomb = lcb,
              #control.inla = list(lincomb.derived.only = FALSE),
              control.predictor = list(link = 1,
                                       compute = TRUE),
...
## Elevation
MyData <- data.frame(
  elevation = seq(from = min(Xm.p$elevation),
             to   = max(Xm.p$elevation),
             length = 100))

MyData
Xmat <- model.matrix(~ elevation,
                     data = MyData)
head(Xmat)
Xmat <- as.data.frame(Xmat)
lcb <- inla.make.lincombs(Xmat)

Mb5.p <- inla(outbreaks ~ elevation,
              lincomb = lcb,
              #control.inla = list(lincomb.derived.only = FALSE),
              control.predictor = list(link = 1,
                                       compute = TRUE),
...
p <- ggplot()
p <- p + geom_point(data = Xm.p, 
                    aes(y = outbreaks, x = elevation),
                    shape = 1, 
                    size = 1)
p <- p + xlab("Elevation") + ylab("Probability of occurrence")
p <- p + theme(text = element_text(size=18)) 
p <- p + geom_line(data = MyData, 
                   aes(x = elevation, 
                       y = mu), 
...
## soil calcium
MyData <- data.frame(
  soil_calcium = seq(from = min(Xm.p$soil_calcium),
                          to   = max(Xm.p$soil_calcium),
                          length = 100))

MyData
Xmat <- model.matrix(~ soil_calcium,
                     data = MyData)
head(Xmat)
Xmat <- as.data.frame(Xmat)
lcb <- inla.make.lincombs(Xmat)

Mb5.p <- inla(outbreaks ~ soil_calcium,
              lincomb = lcb,
              #control.inla = list(lincomb.derived.only = FALSE),
              control.predictor = list(link = 1,
                                       compute = TRUE),
...
p <- ggplot()
p <- p + geom_point(data = Xm.p, 
                    aes(y = outbreaks, x = soil_calcium),
                    shape = 1, 
                    size = 1)
p <- p + xlab("soil_calcium") + ylab("Probability of occurrence")
p <- p + theme(text = element_text(size=18)) 
p <- p + geom_line(data = MyData, 
                   aes(x = soil_calcium, 
                       y = mu), 
...
## soil water
MyData <- data.frame(
  soilwater = seq(from = min(Xm.p$soilwater),
                     to   = max(Xm.p$soilwater),
                     length = 100))

MyData
Xmat <- model.matrix(~ soilwater,
                     data = MyData)
head(Xmat)
Xmat <- as.data.frame(Xmat)
lcb <- inla.make.lincombs(Xmat)

Mb5.p <- inla(outbreaks ~ soilwater,
              lincomb = lcb,
              #control.inla = list(lincomb.derived.only = FALSE),
              control.predictor = list(link = 1,
                                       compute = TRUE),
...
p <- ggplot()
p <- p + geom_point(data = Xm.p, 
                    aes(y = outbreaks, x = soilwater),
                    shape = 1, 
                    size = 1)
p <- p + xlab("soil water") + ylab("Probability of occurrence")
p <- p + theme(text = element_text(size=18)) 
p <- p + geom_line(data = MyData, 
                   aes(x = soilwater, 
                       y = mu), 
...
# Fit of the model.
Pi <- Mb19.c$summary.fitted.values[1:N,"mean"]
...
pred.data <- read.csv(file = "pred_data_1km_plusvars.csv", 
                      header = TRUE,
                      na.strings = "NA",
                      stringsAsFactors = TRUE,
                      dec = ".")
...
summary(pred.data)
pred.nas1 <- pred.data[is.na(pred.data$soil_pH),]
pred.nas2 <- pred.data[is.na(pred.data$soilwater),]
pred.nas <- rbind (pred.nas1, pred.nas2)

pred.data <- pred.data[!is.na(pred.data$soil_pH),]
...
list.pred[[21]] <- pred.data[200001:210475,]
...
xyz.data.mean <- pred.result[,c(36,37,32)]
xyz.data.sd <- pred.result[,c(36,37,33)]
xyz.data.low <- pred.result[,c(36,37,34)]
xyz.data.up <- pred.result[,c(36,37,35)]
...
###########################################################################
# Extract the test values from the predicted rasters.
...
head(rasValue)
omm.I1 <- length(rasValue[(rasValue$rasValue<0.58),])/nrow(rasValue)
...
# Fit of the model.
Pi <- Mb10.c$summary.fitted.values[1:N,"mean"]
Fit01 <- ifelse(Pi >= 0.4, 1, 0)
table(Fit01,mydata$outbreaks )
...
Z <- table(mydata$outbreaks, Fit01)
Z
CorrectCount <- sum(diag(Z))
TotalCount   <- sum(Z)
Count <- CorrectCount /  TotalCount
Count # [1] 
